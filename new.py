...
def process_document(uploaded_file: UploadedFile) -> list[Document]:
    ...
    temp_file.close()  # Close the file so it can be safely deleted later

    loader = PyMuPDFLoader(temp_file.name)
    docs = loader.load()
    os.unlink(temp_file.name)  # Now safe to delete
...
def re_rank_cross_encoders(documents: list[str], query_prompt: str) -> tuple[str, list[int]]:
    ...
    Args:
        ...
        query_prompt: The user's original query prompt.
    ...
    ranks = encoder_model.rank(query_prompt, documents, top_k=3)
    for rank in ranks:
...
if __name__ == "__main__":
    ...
    with st.sidebar:
        st.set_page_config(page_title="RagNeoIQ Question Answer Bot")
    ...
    # AREA SECTION TO ANSWER THE QUESTION SUPPLIED 
    ...
    if ask and prompt:
        ...
        documents = results.get("documents", [[]])[0]  # Get list of docs for first query result
        relevant_text, relevant_text_ids = re_rank_cross_encoders(documents, prompt)
